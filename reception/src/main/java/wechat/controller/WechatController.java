            ...
            switch (message.getMsgType()) {
                case "event":
                    if (message.getEvent().equals("subscribe")) {
                        ...
                        result.setContent("Hey,您终于来了！\n恭喜您找到新鲜至净空气提供者:\n果麦新风\n这里只跟您分享室内好空气的秘密。\n回复【合作】洽谈合作项\n回复【兑换】兑换礼品卡\n回复【赠送】所要小礼物\n您也可以点击下方菜单自助查阅所需要的内容,感谢您的支持,祝您生活愉快!");
                        String xml = content.toXML(result);
                        return xml;
                    }
                    if ("gmair".equals(message.getEventKey())) {
                    ...
                    break;
                case "text":
                    if ("合作".equals(message.getContent())) {
                        content.alias("xml", TextOutMessage.class);
                        TextOutMessage result = initialize("市场合作请发邮件至pr@gmair.net", message);
                        String xml = content.toXML(result);
                        return xml;
                    }
                    if ("兑换".equals(message.getContent())) {
                        content.alias("xml", TextOutMessage.class);
                        TextOutMessage result = initialize("http://one.fw1860.com/recinzaixiantihuoxitongimages/wxth/njgm/wxth.aspx", message);
                        String xml = content.toXML(result);
                        return xml;
                    }
